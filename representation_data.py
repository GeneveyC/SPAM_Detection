...
def add_expert_features(text,df):
    '''
    ...
    for i in np.arange(0,len(df)):
        df.loc[i,'length'] = len(text[i])
    df['number']=0
    for i in range(0,len(df)):
        df.loc[i,'number']=contain_number(text[i])
    df['maj']=0
    for i in range(0,len(df)):
        df.loc[i,'maj']=contain_maj(text[i])
    df['caract']=0
    for i in range(0,len(df)):
        df.loc[i,'caract']=contain_caract(text[i])
    return df
...
def get_data(option=1,rs=10,rh=0.2,Mt=4000,random_state_=None):
    ...
    text_train,text_test,y_train,y_test=train_test_split(text,label,train_size=train_size_,random_state=random_state_,shuffle=True)
    # Transform data into numerical matrix (dtm)
    vect=CountVectorizer()
    vect.fit(text_train)
    X_train_dtm=vect.transform(text_train)
    X_test_dtm=vect.transform(text_test)
    ...
    if option==1 :
        ...
        if option == 2 :
            ...
            df_train=add_expert_features(text_train.as_matrix(),df_train)
            df_test=add_expert_features(text_test.as_matrix(),df_test)
...
if __name__=="__main__":
    ...
    X_train,X_test,y_train,y_test,feature_names=get_data(option=3,rs=10,rh=0.8,random_state_=1,Mt=4000)
    #print X_train
    #print X_test
    #print y_train.shape
    #print y_test.shape
    ...
    # First representation fo the data :
#    df1=pd.DataFrame(X_train,columns=feature_names)
#    alea_col=shuffle(df1.columns)[0:6]
#    pd.DataFrame(df1.as_matrix(columns=alea_col),columns=alea_col).head(5)
    
    
    #Second representation :
#    df_train=pd.DataFrame(np.concatenate((y_train.T[:,None],X_train),axis=1),columns=np.concatenate((np.array(['label']),feature_names),axis=0))
#
#    spam_ratio=get_spam_ratio(df_train)
#    print spam_ratio
